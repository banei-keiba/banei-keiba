...
| results | 307,849 | |
| payouts | 331,427 | |
| odds（単複） | 307,801 | **全 33,493 レース分**（2026-08-06 完了） |
| combo_odds | 6,178 | **ほぼ未取得**（86 レース×券種 / 対象 約134,000）。三連単は 1着馬ごとに 1 リクエスト必要で重い |
...
- [x] オッズのバックフィル手順 `scripts/backfill-local.sh`（Actions では不可・後述）
...
よう注意する（`scripts/backfill-local.sh` は最新バックアップを取得してから始める）。
...
- 過去分は `scripts/backfill-local.sh` で**手元から**少しずつ

に分けた。

##### 過去分を Actions で回せない理由

オッズパークには**送信元ごとの累積クォータ**がある（時間窓ベース）。2026-08-06 の実測:
...
ランナー 2 回目が 253 件で落ちたのは、1 回目の失敗開始から 30 分しか空いておらず、
直前 1 時間の窓に 1 回目の約 1,700 リクエスト（リトライ含む）が残っていたため。
残量ゼロから始まったので、窓がスライドして空いた分だけ通った。

間隔を広げても効かないのは、制限が瞬間レートではなく累積本数だから。
また **GitHub Actions の IP は全ユーザー共有**で、他人のスクレイパーも同じ枠を消費する。
ランナー側の枠が小さく、実行ごとにばらつくのはこれで説明できる。

手元からは 2,400 件を連続で完走できたため送信元によって枠が違うが、
**手元が無制限という証拠はない**（2,400 までしか試していない）。区切って回し、
実行の合間を空けること。

日次収集は直近 30 日ぶん（開催日で 10〜12 レース程度）しか取らないため、
この制限には当たっていない。
