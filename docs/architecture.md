...
オッズパークは GitHub Actions の IP からのアクセスに HTTP 500 を返す。2026-08-06 の検証:

| 実行元 | 間隔 | 結果 |
|---|---|---|
| GitHub ランナー | 1秒 | 1,400 件あたりから全リクエストが 500 |
| GitHub ランナー | 2秒 | **200 件で失敗開始**（遅くしても悪化） |
| 手元（WSL2） | 1秒 | **400 件連続で失敗ゼロ** |

決め手は、ランナーが 500 を返された 2022-01-02 / 01-03 の全 24 レースが手元からは
完全に取得できたこと。レートでもデータでもなく**送信元による制限**である。

間隔を広げるほど悪化したのは直感に反するが、いずれにせよ Actions では完走できない。
日次収集は直近 30 日ぶん（開催日で 10〜12 レース程度）しか取らないため、この制限には
当たっていない。
