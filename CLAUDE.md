...
### オッズパークは GitHub Actions の IP を拒否する

**バックフィルを Actions で回してはいけない。** 2026-08-06 に検証した結果:

| 実行元 | 間隔 | 結果 |
|---|---|---|
| GitHub ランナー | 1秒 | 1,400 件あたりから全リクエストが HTTP 500 |
| GitHub ランナー | 2秒 | **200 件で失敗開始**（遅くしても改善せず、むしろ悪化） |
| 手元（WSL2） | 1秒 | **400 件連続で失敗ゼロ** |

決め手は、ランナーが 500 を返された 2022-01-02 / 01-03 の全 24 レースが、
手元からは完全に取得できたこと。レートでもデータでもなく**送信元による制限**。

そのため `scripts/backfill-local.sh` を使って**手元から**回す。

```bash
./scripts/backfill-local.sh 1000 1.0   # 最新バックアップを取得 → 収集 → 検証 → 書き戻し
```
...
最後まで到達しないと何も残らない。スクレイパー側は連続 20 件失敗したら
**自分で中断して正常終了**するようにしてある（例外で落とすと後続の保存手順が走らないため）。
失敗したレースは meta を書かないので、次回の実行で再挑戦される。
